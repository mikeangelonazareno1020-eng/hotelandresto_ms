...
class FrontdeskController extends Controller
{
    public function bookingIndex(Request $request)
    {
        ...
        return view('frontdesk.frontdeskBooking', [
    ...
    public function bookingCreate()
    {
        ...
        // Pass to view
        return view('frontdesk.frontdeskBookingForm', compact('rooms'));
    }

    // Frontdesk Reports page
    public function reports()
    {
        $reservations = RoomReservation::query()->latest()->get();
        $today = \Carbon\Carbon::today('Asia/Manila')->format('Y-m-d');
        $summary = [
            'total_reservations' => $reservations->count(),
            'fully_paid' => $reservations->where('payment_status', 'Fully Paid')->count(),
            'not_fully_paid' => $reservations->where('payment_status', 'Not Fully Paid')->count(),
            'checkins_today' => $reservations->where('checkin_date', $today)->count(),
            'checkouts_today' => $reservations->where('checkout_date', $today)->count(),
            'total_amount' => $reservations->sum('total_amount'),
            'net_amount' => $reservations->sum('net_amount'),
        ];
        return view('frontdesk.frontdeskReports', compact('reservations', 'summary'));
    }

    // Transport services list (frontdesk)
    public function transportServices()
    {
        $services = TransportService::orderByDesc('created_at')->get();
        return view('frontdesk.transport.frontdeskTransportServices', compact('services'));
    }

    public function transportServiceCreate()
    {
        return view('frontdesk.transport.frontdeskTransportBookPage');
    }

    public function transportServiceStore(Request $request)
    {
        $data = $request->validate([
            ...
            // Accept either composed datetime or separate date/time
            'pickup_eta' => 'nullable|date',
            'pickup_date' => 'nullable|date',
            'pickup_time' => 'nullable|date_format:H:i',

            'pickup_location' => 'nullable|string|max:120',
            'pickup_lat' => 'nullable|numeric|between:-90,90',   // ✅ match form field
            'pickup_lng' => 'nullable|numeric|between:-180,180', // ✅ match form field
        ...
        // Compose pickup_eta if not provided but date/time exist
        if (empty($data['pickup_eta'])) {
            $d = $data['pickup_date'] ?? null;
            $t = $data['pickup_time'] ?? null;
            if ($d && $t) {
                try {
                    $data['pickup_eta'] = \Carbon\Carbon::parse("{$d} {$t}")->toDateTimeString();
                } catch (\Throwable $e) {
                    return back()->withErrors(['pickup_eta' => 'Invalid pickup date/time'])->withInput();
                }
            }
        }

        // Final guard: require pickup_eta after composition
        if (empty($data['pickup_eta'])) {
            return back()->withErrors(['pickup_eta' => 'Pickup date and time are required.'])->withInput();
        }
        ...
        $payload = [
            ...
            // ✅ aligned with form hidden fields
        ...
        TransportService::create($payload);

        return redirect()
            ->route('frontdesk.transport.index')
            ->with('success', 'Transport service created successfully.');
    }


    // Lightweight search endpoint to look up reservations by reservation_id prefix
    public function searchReservations(\Illuminate\Http\Request $request)
    {
        $q = trim((string) $request->query('q', ''));
        if ($q === '') {
            return response()->json([]);
        }
        $rows = RoomReservation::query()
            ->select('reservation_id', 'first_name', 'last_name', 'room_number', 'checkin_date', 'checkout_date', 'payment_status')
            ->where('reservation_id', 'like', "$q%")
    ...
    public function bookingStore(Request $request)
    {
        ...
        ")->AUTO_INCREMENT ?? 1;
            ...
            $reservation = RoomReservation::create([
                ...
                'payment_status' => (function () use ($payload) {
                    $totalDue = (float) ($payload['total_due'] ?? 0);
                    return $totalDue <= 0 ? 'Fully Paid' : 'Not Fully Paid';
                })(),
            ...
            Receipt::create([
                ...
                // 🧾 Staff / Admin
                'issued_by' => $payload['issued_by'] ?? session('user_name') ?? 'Frontdesk Staff',
            ...
            return redirect()->route('frontdesk.booking')->with('alert', [
                'type' => 'success',
                'title' => 'Success',
    ...
    public function checkinPage($reservationId)
    {
        ...
        return view('frontdesk.frontdeskBookingCheckinPage', compact(
            'reservation',
            'receipts',
            'addedAmenityNames',
            'downpaymentAmount',
    ...
    public function bookingCheckIn()
    {
        ...
        try {
            ...
            if ($reservation->net_amount > 0) {
                $reservation->payment_status = 'Not Fully Paid';
            } else {
    ...
    public function checkoutPage($reservationId)
    {
        ...
        return view('frontdesk.frontdeskBookingCheckoutPage', compact(
            'reservation',
            'receipts',
            'addedAmenityNames',
            'downpaymentAmount',
    ...
    public function cancelPage($reservationId)
    {
        ...
        return view('frontdesk.frontdeskBookingCancelPage', compact(
            'reservation',
            'receipts',
            'addedAmenityNames',
            'downpaymentAmount',
    ...
    public function bookingCancel(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route(route: 'frontdesk.booking')->with('alert', [
                'type' => 'success',
    ...
    public function detailsPage($reservationId)
    {
        ...
        // ✅ Return same structure & naming as Check-In
        return view('frontdesk.frontdeskBookingDetailsPage', compact(
            'reservation',
            'receipts',
            'addedAmenityNames',
            'downpaymentAmount',
    ...
    public function saveAdditions(Request $request)
    {
         ...
         */
        $paymentStatus = $netAmount > 0 ? 'Not Fully Paid' : 'Fully Paid';
    ...
    public function processPayment(Request $request)
    {
        ...
        $paymentStatus = $newBalance <= 0 ? 'Fully Paid' : 'Not Fully Paid';
    ...
    public function roomsIndex()
    {
        ...
        return view('frontdesk.frontdeskRooms', compact('rooms'));
    }
