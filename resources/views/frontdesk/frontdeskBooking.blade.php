...
@section('title', 'Hotel Room Check In/Out & Reservations')
...
>
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <p class="text-[11px] text-gray-600">Monitor and manage all hotel bookings</p>
        </div>
        <a href="{{ route('frontdesk.booking.create') }}"
    ...
    @php
        ...
        $booked = $reservations->where('reservation_status', 'Booked')->count();
        $checkedIn = $reservations->where('reservation_status', 'Checked In')->count();
        $checkedOut = $reservations->where('reservation_status', 'Checked Out')->count();
        $cancelled = $reservations->where('reservation_status', 'Cancelled')->count();
        $noShow = $reservations->where('reservation_status', 'No Show')->count();
    @endphp
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-4 text-center text-[11px] font-[Poppins]">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-2.5 shadow-sm flex flex-col items-center">
            <i data-lucide="calendar-plus" class="w-4 h-4 text-blue-600 mb-1"></i>
            <p class="text-gray-500 text-[10px] font-medium">Booked</p>
            <h2 class="text-base font-bold text-blue-600">{{ $booked }}</h2>
        </div>
        <div class="bg-white border border-gray-200 rounded-lg p-2.5 shadow-sm flex flex-col items-center">
            <i data-lucide="log-in" class="w-4 h-4 text-green-600 mb-1"></i>
            <p class="text-gray-500 text-[10px] font-medium">Checked In</p>
            <h2 class="text-base font-bold text-green-600">{{ $checkedIn }}</h2>
        </div>
        <div class="bg-white border border-gray-200 rounded-lg p-2.5 shadow-sm flex flex-col items-center">
            <i data-lucide="log-out" class="w-4 h-4 text-amber-600 mb-1"></i>
            <p class="text-gray-500 text-[10px] font-medium">Checked Out</p>
            <h2 class="text-base font-bold text-amber-600">{{ $checkedOut }}</h2>
        </div>
        <div class="bg-white border border-gray-200 rounded-lg p-2.5 shadow-sm flex flex-col items-center">
            <i data-lucide="x-circle" class="w-4 h-4 text-red-600 mb-1"></i>
            <p class="text-gray-500 text-[10px] font-medium">Cancelled</p>
            <h2 class="text-base font-bold text-red-600">{{ $cancelled }}</h2>
        </div>
        <div class="bg-white border border-gray-200 rounded-lg p-2.5 shadow-sm flex flex-col items-center">
            <i data-lucide="user-x" class="w-4 h-4 text-gray-700 mb-1"></i>
            <p class="text-gray-500 text-[10px] font-medium">No Show</p>
            <h2 class="text-base font-bold text-gray-700">{{ $noShow }}</h2>
        </div>
    </div>
                    ...
                    <tr class="hover:bg-gray-50 transition"
                        ...
                        <td class="px-3 py-2">
                            <div class="flex flex-wrap gap-1.5">
                                <a href="{{ route('frontdesk.booking.detailspage', $reservation->reservation_id) }}"
                               class="inline-flex items-center justify-center text-gray-600 hover:text-[#B0452D] transition">
                                ...
                                @if($reservation->reservation_status === 'Booked')
                                    <a href="{{ route('frontdesk.booking.checkinpage', $reservation->reservation_id) }}"
                                    ...
                                    <a href="{{ route('frontdesk.booking.cancelpage', $reservation->reservation_id) }}"
                                ...
                                @elseif($reservation->reservation_status === 'Checked In')
                                    <a href="{{ route('frontdesk.booking.checkoutpage', $reservation->reservation_id) }}"
...
document.addEventListener('alpine:init', () => {
    Alpine.data('bookingList', () => ({
        ...
        roomTypeFilter: '',
        activeTab: 'All',
        tabs: ['All', 'Booked', 'Checked In', 'Checked Out', 'Cancelled', 'No Show'],
