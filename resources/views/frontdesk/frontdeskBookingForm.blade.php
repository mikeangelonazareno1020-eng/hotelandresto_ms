...
>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        ...
        <a href="{{ route('frontdesk.booking') }}" 
        ...
        <form @submit.prevent="validateAndSubmit" class="space-y-8">
            ...
            <section>
                ...
                <div class="flex justify-end gap-3 mt-6 border-t border-amber-100 pt-4">
                    ...
                    <button type="button"
                            @click="showPayment = true; $nextTick(() => lucide?.createIcons?.())"
                            class="px-5 py-2 rounded-lg text-sm font-semibold bg-[#3B9441] hover:bg-[#2E7D32] text-white shadow transition">
                        Make Payment
                    </button>
                </div>
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-[#FFD600]/40 p-5 flex flex-col max-h-[90vh]">
          ...
          <div class="flex-1 overflow-y-auto no-scrollbar">
            <div class="bg-[#FFFDF8] border border-[#FFD600]/50 rounded-xl p-4 text-sm">
              <div class="text-center mb-2">
                ...
                <p class="text-xs text-gray-500">Reservation Receipt</p>
              </div>

              <div class="grid grid-cols-2 gap-x-2 gap-y-0.5 text-xs text-gray-600 mb-2">
                <div>Room</div>
                <div class="text-right font-medium text-[#3B9441]" x-text="(selectedRoomType || '-') + (selectedRoom ? ' - '+selectedRoom : '')"></div>
                <div>Nights</div>
                <div class="text-right" x-text="total_days"></div>
                <div>Check-in</div>
                <div class="text-right" x-text="checkinDate"></div>
                <div>Check-out</div>
                <div class="text-right" x-text="checkoutDate"></div>
              </div>
              ...
              <div class="text-[12px] space-y-1">
                <div class="flex justify-between">
                  <span>Room Fee</span>
                  <span class="font-semibold text-[#2E2E2E]">₱<span x-text="reservationFee.toFixed(2)"></span></span>
                </div>
                <div class="flex justify-between">
                  <span>Added Amenities</span>
                  <span class="font-semibold text-[#2E2E2E]">₱<span x-text="addedAmenitiesFee.toFixed(2)"></span></span>
                </div>
                <div class="flex justify-between border-t border-dashed border-[#FFD600]/50 pt-2">
                  <span class="text-[#C45E25] font-semibold">Total</span>
                  <span class="text-[#3B9441] font-semibold">₱<span x-text="totalAmount.toFixed(2)"></span></span>
                </div>
                <template x-if="paymentAmount > 0">
                  <div class="flex justify-between text-[#D92332]">
                    <span>Payment Entered</span>
                    <span class="font-semibold">-₱<span x-text="(Number(paymentAmount)||0).toFixed(2)"></span></span>
                  </div>
                </template>
                <div class="flex justify-between border-t border-dashed border-[#FFD600]/50 pt-2">
                  <span class="font-semibold">Total Due</span>
                  <span class="text-[#3B9441] font-semibold">₱<span x-text="totalDue.toFixed(2)"></span></span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-3 mt-4">
            ...
            <div>
              ...
              <input type="number" step="0.01" x-model.number="paymentAmount" @input="onPaymentFieldsChange()" class="w-full rounded-lg border px-3 py-1.5 text-xs focus:ring-[#FFD600] focus:border-[#FFD600]" />
              <span x-show="paymentAmountError" x-text="paymentAmountError" class="text-[11px] text-red-600"></span>
            </div>

            <div>
              <label class="text-xs text-gray-600">Method</label>
              <select x-model="paymentMethod" @change="onPaymentFieldsChange()" class="w-full rounded-lg border px-3 py-1.5 text-xs focus:ring-[#FFD600] focus:border-[#FFD600]">
            ...
            <!-- Cash Fields -->
            <div x-show="paymentMethod==='Cash'" class="grid grid-cols-2 gap-2">
              <div class="relative pb-3">
                <label class="text-xs text-gray-600">Amount Tendered</label>
                <input type="number" step="0.01" x-model.number="amountTendered" @input="calculateChange()" class="w-full rounded-lg border px-3 py-1.5 text-xs focus:ring-[#FFD600]" />
                <span x-show="(Number(amountTendered||0) < Number(paymentAmount||0))" class="absolute right-2 -bottom-1 text-[11px] text-red-600">Insufficient</span>
              </div>
              <div>
                <label class="text-xs text-gray-600">Change</label>
                <div class="h-[34px] flex items-center px-3 rounded-lg border bg-gray-50 text-xs font-semibold text-[#3B9441]">₱<span x-text="(Number(changeDue)||0).toFixed(2)"></span></div>
              </div>
            </div>

            <!-- Digital Fields (GCash/PayMaya/Bank) -->
            <div x-show="['GCash','PayMaya','Bank Transfer'].includes(paymentMethod)" class="grid grid-cols-2 gap-2">
              <div>
                <label class="text-xs text-gray-600">Reference No.</label>
                <input type="text" x-model="paymentDetails.reference_number" class="w-full rounded-lg border px-3 py-1.5 text-xs focus:ring-[#FFD600]" />
              </div>
              <div>
                <label class="text-xs text-gray-600">Transaction ID</label>
                <input type="text" x-model="paymentDetails.transaction_id" class="w-full rounded-lg border px-3 py-1.5 text-xs focus:ring-[#FFD600]" />
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-3 pt-3 border-t border-[#FFD600]/30 mt-4 sticky bottom-0 bg-white">
            ...
            <button type="button"
                    :disabled="paymentAmountError || !paymentMethod || (paymentMethod==='Cash' && ((Number(amountTendered)||0) < (Number(paymentAmount)||0)))"
                    :class="(paymentAmountError || !paymentMethod || (paymentMethod==='Cash' && ((Number(amountTendered)||0) < (Number(paymentAmount)||0)))) ? 'opacity-60 cursor-not-allowed bg-gray-300 text-gray-600' : 'bg-[#3B9441] text-white hover:bg-[#2E7D32]'
                    "
                    class="flex items-center gap-1 px-4 py-1.5 rounded-lg transition"
...
function reservationForm() {
    return {
        ...
        validateAndSubmit() {
            ...
            form.method = 'POST';
            form.action = "{{ route('frontdesk.booking.store') }}";
