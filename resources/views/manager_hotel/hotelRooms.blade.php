...
>
    ...
    >
        ...
        <div class="mt-6 flex justify-end gap-3">
            <!-- Edit button -->
            <a
                :href="'{{ url('/hotel/rooms') }}/' + (selectedRoom?.room_number ?? '') + '/edit'"
                class="px-4 py-2 bg-amber-600 text-white hover:bg-amber-700 rounded-lg text-[12px] font-medium flex items-center gap-1 transition"
            >
                <i data-lucide="pencil" class="w-4 h-4"></i>
                Edit Details
            </a>
            <!-- 🧰 Maintenance -->
            <template x-if="formatStatus(selectedRoom.room_status) === 'Vacant'">
                <button 
                    @click="markRoom('Maintenance')" 
                    class="px-4 py-2 bg-blue-100 text-blue-700 hover:bg-blue-200 rounded-lg text-[12px] font-medium flex items-center gap-1 transition">
            ...
            <template x-if="formatStatus(selectedRoom.room_status) === 'Vacant'">
                <button 
                    @click="markRoom('Out of Service')" 
                    class="px-4 py-2 bg-red-100 text-red-700 hover:bg-red-200 rounded-lg text-[12px] font-medium flex items-center gap-1 transition">
            ...
            <!-- ✅ Make Vacant -->
            <template x-if="['Maintenance', 'Out of service', 'Checked Out'].includes(formatStatus(selectedRoom.room_status))">
                <button 
        ...
        markRoom(status) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `{{ route('hotelmanager.rooms.updateStatus', ':room_number') }}`.replace(':room_number', this.selectedRoom.room_number);
